    ...
    async def fix_test_failures(
        ...
        **kwargs
        ...
        system_message = (
            ...
            "FIXTURE RULES — MANDATORY, NON-NEGOTIABLE:\n"
            "The @Before and @After methods are the ONLY place where fixture setup and teardown\n"
            "may occur. This rule applies to any REST API under test, with NO exceptions.\n\n"
            "YOU MUST:\n"
            "- Place ALL resource creation (POST requests, seed data) needed by any test inside @Before.\n"
            "- Place ALL resource cleanup (DELETE requests, RestAssured.reset()) inside @After,\n"
            "  in reverse order of @Before.\n"
            "- Ensure @Before and @After together form a complete, symmetric lifecycle:\n"
            "  every resource created in @Before must be deleted in @After, nothing more, nothing less.\n\n"
            "YOU MUST NOT:\n"
            "- Add any HTTP request that creates or deletes data inside individual @Test methods.\n"
            "- Add teardown calls (DELETE, cleanup) inside individual @Test methods.\n"
            "- Duplicate in a @Test any fixture logic that already exists in @Before/@After.\n\n"
            "FIXTURE DIAGNOSIS RULE: When fixing a failing test, first ask:\n"
            "  'Is this test failing because the fixture in @Before is incomplete\n"
            "   (e.g. a required resource does not exist yet when the test runs)?'\n"
            "  -> If YES: expand @Before to create the missing resource (using the correct POST\n"
            "             endpoint from the API spec), and add the matching DELETE to @After.\n"
            "             Do NOT touch the @Test body.\n"
            "  -> If NO: fix only the assertion or request construction inside the @Test body.\n\n"
            "FORBIDDEN PATTERN — never produce this, regardless of the API being tested:\n"
            "  @Test\n"
            "  public void test_get_resource_success() {\n"
            "      given().post(\"/api/v1/<resource>\");            // FORBIDDEN: belongs in @Before\n"
            "      given().get(\"/api/v1/<resource>\").then().statusCode(200);\n"
            "      given().delete(\"/api/v1/<resource>\");          // FORBIDDEN: belongs in @After\n"
            "  }\n\n"
            "CORRECT PATTERN — always produce this structure:\n"
            "  @Before\n"
            "  public void setupTestData() {\n"
            "      RestAssured.baseURI = \"<base_url_from_project_context>\";\n"
            "      given().<method>(\"/api/v1/<resource>\"); // ALL fixture creation here\n"
            "  }\n"
            "  @After\n"
            "  public void cleanupTestData() {\n"
            "      given().delete(\"/api/v1/<resource>\"); // ALL cleanup here, reverse order\n"
            "      RestAssured.reset();\n"
            "  }\n"
            "  @Test\n"
            "  public void test_get_resource_success() {\n"
            "      given().get(\"/api/v1/<resource>\").then().statusCode(200); // test only\n"
            "  }\n"
            "The placeholders <resource>, <method>, and <base_url_from_project_context> must be\n"
            "replaced with the actual endpoints and URL from the API under test.\n\n"
            "SERVER NON-COMPLIANCE RULE:\n"
            "When a test fails because the live server returns a status code different from the spec\n"
            "(e.g. server returns 500 but spec requires 400), the correct fix is:\n"
            "  @Ignore(\"Server returns <actual> but spec requires <expected> — server non-compliant\")\n"
            "NEVER modify the expected status code to match wrong server behavior.\n"
            "NEVER remove an existing @Ignore annotation that is already justified.\n\n"
            "IMPORTANT: Return ONLY the corrected Java code without any explanations or markdown formatting."
        )
        ...
        prompt = (
            ...
            f"Test Failures:\n{failures}\n"
            f"{min_tests_clause}\n"
            "FIXTURE INTEGRITY CHECK: Before returning your fix, verify that:\n"
            "  1. Every POST/seed call is inside @Before (setupTestData), NOT inside any @Test.\n"
            "  2. Every DELETE/cleanup call is inside @After (cleanupTestData), NOT inside any @Test.\n"
            "  3. No @Test method contains setup or teardown logic.\n"
            "  If a test was failing due to missing fixture data, you fixed @Before — not the @Test.\n\n"
