...
class CodeSanitizer(LoggerMixin):
    ...
    def enforce_restassured_lifecycle(self, java_code: str) -> str:
        ...
        2. Inside ``@BeforeClass`` / ``setupTestData()`` and ``@Before`` / ``setupTestData()``:
           the body is PRESERVED as-is, but:
           a. Any bare ``baseURI =`` is replaced with ``RestAssured.baseURI =``.
           b. ``RestAssured.baseURI = <url>;`` is ensured to be the FIRST statement
              (prepended if not already first, removing any duplicate further down).

        3. Inside ``@After`` / ``cleanupTestData()``:
           the body is PRESERVED as-is, but ``RestAssured.reset();`` is ensured
           to be the LAST statement (appended, removing any duplicate earlier).

        This preserves all fixture creation (POST) and teardown (DELETE) logic
        generated by the LLM while enforcing the mandatory first/last statements.
    ...
    def _fix_lifecycle_methods(self, lines: list, base_url: str) -> list:
        ...
        * For ``cleanupTestData``: replace the entire body with a single
        ...
        while i < len(result):
            ...
            if matched_lifecycle in (BEFORE_CLASS, BEFORE):
                # Preserve the entire body but ensure:
                # 1. RestAssured.baseURI = <url>; is the FIRST statement
                ...
                # Check if RestAssured.baseURI is already the first non-empty statement
                first_stmt_idx = None
                for idx, ln in enumerate(fixed_body):
                    if ln.strip():
                        first_stmt_idx = idx
                        break

                has_baseuri_first = (
                    first_stmt_idx is not None and
                    'RestAssured.baseURI =' in fixed_body[first_stmt_idx]
                )

                if not has_baseuri_first:
                    # Remove any existing RestAssured.baseURI assignment from body
                    fixed_body = [
                        ln for ln in fixed_body
                        if 'RestAssured.baseURI =' not in ln.strip() and
                           'baseURI =' not in ln.strip()
                    ]
                    # Prepend as first statement
                    fixed_body.insert(0, f'{body_indent}RestAssured.baseURI = {base_url};')
                ...
                # Preserve the entire body but ensure RestAssured.reset() is the LAST statement
                body_lines = list(result[open_brace_idx + 1:close_brace_idx])
